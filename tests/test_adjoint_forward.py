...
class TestMagnusAdjointForward:
    ...
    def test_magnus_adjoint_forward_nonhomogeneous_magnus(self):
        def system_func(t, params):
            t_tensor = torch.as_tensor(t)
            if t_tensor.ndim == 0:
                A = torch.eye(2) * t_tensor
                g = torch.ones(2) * t_tensor
            else:
                A = torch.eye(2).unsqueeze(0).expand(t_tensor.shape[0], -1, -1) * t_tensor.unsqueeze(-1).unsqueeze(-1)
                g = torch.ones(t_tensor.shape[0], 2) * t_tensor.unsqueeze(-1)
            return A, g
    ...
    def test_magnus_adjoint_forward_with_params(self):
        ...
        def system_func(t, params):
            t_tensor = torch.as_tensor(t)
            if t_tensor.ndim == 0:
                A = torch.eye(2) * t_tensor * params['p']
            else:
                A = torch.eye(2).unsqueeze(0).expand(t_tensor.shape[0], -1, -1) * t_tensor.unsqueeze(-1).unsqueeze(-1) * params['p']
            return A
